...
async def list_images():
    ...
    try:
        ...
        files = [f for f in os.listdir(UPLOAD_FOLDER) if
                 os.path.isfile(os.path.join(UPLOAD_FOLDER, f)) and f.endswith(('.png', '.jpg', '.jpeg', '.gif'))]
        return {"images": files}
    except Exception as e:
...
async def add_comment(comment_data: CommentCreate, image_name: str = Path(...)):
    ...
    try:
        ...
        await cursor.close()
        return {"message": "Comment added successfully (potentially via SQL Injection!)", "image_name": image_name,
                "comment": comment_text}  # "id": last_id
